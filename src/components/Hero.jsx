...
// Dummy images - replace these with your actual project images
const SLIDER_IMAGES = [
  Image1,
  Image2,
  Image3,
...
export default function Hero({ onEnquire }) {
  ...
  const nextSlide = () => setCurrent(current === SLIDER_IMAGES.length - 1 ? 0 : current + 1);
  const prevSlide = () => setCurrent(current === 0 ? SLIDER_IMAGES.length - 1 : current - 1);
  ...
  return (
    <>
      <style>{`
        ...
        @media (max-width: 900px) {
          .hero-grid { grid-template-columns: 1fr !important; }
          .hero-right  { height: 70vw !important; min-height: 400px !important; }
          .hero-left   { padding: 48px 24px !important; }
          .hero-stats  { flex-wrap: wrap; gap: 16px !important; }
        }
      `}</style>
      ...
      >
        ...
        >
          ...
          {/* Slider Controls */}
          <div style={{ position: "absolute", bottom: 40, right: 40, zIndex: 10, display: "flex", gap: 10 }}>
            <button 
              onClick={prevSlide}
              style={{ background: "rgba(0,0,0,0.4)", border: "none", color: "white", width: 40, height: 40, cursor: "pointer", backdropFilter: "blur(4px)" }}
            >
              &#10094;
            </button>
            <button 
